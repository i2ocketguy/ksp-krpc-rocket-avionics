...
# Ki = 2.0*Kp/Tu
# Kd = 2.0*Kp/Tu
vert_vel_controller = controllers.PID(0.0, 0.25, 0.01, 0, new_throttle_limit, 1.0, deadband=0.005)
vert_vel_controller.set_point = -0.02  # vertical velocity target, m/s
alt_controller = controllers.PID(0.0, 1, 0.01, 0.0, min_output=-20, max_output=10)
alt_controller.set_point = telem.apoapsis()
slam_controller = controllers.PID(0.0, 1, 0.1, 0.0, 0.0, 1.0, deadband=0.005)
slam_controller.set_point = 0.3
roll_controller = controllers.PID(0.0, 1.0, 0.0, 0.1,-20, 20, 1.0)
dist_controller = controllers.PID(0.0, 0.05, 0.001, 0.01, -20.0, 20.0, deadband=0.005, rate_limit=0.5)
hvel_controller = controllers.PID(0.0, 1.0, 0.1, 0.0, -60.0, 40.0, deadband=0.005)
...
while vessel.situation != status:
    ...
    if mode == 2:
        ...
        if telem.surface_altitude() < 1500 or distance_to_pad < 100:
            ...
            print(f"Mode 2a Outputs: {distance_to_pad:.2f}, {current_horizontal_velocity:.2f}, {distance_pitch:.2f}, {velocity_pitch:.2f}")
            ...
            distance_pitch = dist_controller.update(distance_to_pad)
            current_horizontal_velocity = vel_sign*vessel.flight(vessel.orbit.body.reference_frame).horizontal_speed
            hvel_controller.set_point = 100.0 * (1.0 - np.exp(-distance_to_pad/1000))
            velocity_pitch = -hvel_controller.update(current_horizontal_velocity)
            pitch_input = distance_pitch + velocity_pitch
            ...
            print(f"Mode 2b Outputs: {distance_to_pad:.2f}, {current_horizontal_velocity:.2f}, {hvel_controller.set_point:.2f}, {distance_pitch:.2f}, {velocity_pitch:.1f}")

        if tb > -0.75 and telem.surface_altitude() < 1500 and burn_flag is False:
        ...
        if telem.surface_altitude() < 120 or telem.vertical_vel() > -5:
            ...
            hvel_controller.set_gains(1.0, 0.05, 0.01)
    ...
    if mode == 3:
        ...
        print(f"Mode 3 Outputs: {distance_to_pad:.2f}, {telem.horizontal_vel():2f}, {pitch:.2f}, {vert_vel_controller.set_point:.2}")
