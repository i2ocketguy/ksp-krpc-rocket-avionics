...
def pitch_maneuver(prop_meco_condition, mission_params, telem, vessel, conn, sc,
    ...
    while not meco:
        utils.abort_system(sc.is_abort_installed, sc.abort_criteria, vessel,
                           mission_params, conn, "Minerva-I", telem_viz=telem_viz)
        if not mission_params.maxq_exit:
            sas.maxQ(mission_params, telem, vessel, maxq_thrust_control, telem_viz=telem_viz)
...
sas.roll_program(mission_params, telem, vessel, conn, minerva, telem_viz=telem_viz)
...
sas.meco(vessel, minerva, telem_viz=telem_viz)
...
second = final_stage.close_loop_guidance(second, mission_params, telem, 120, mission_params.target_heading, pid_input=final_pitch_control, telem_viz=telem_viz)
